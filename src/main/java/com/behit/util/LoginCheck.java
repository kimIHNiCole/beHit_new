...
public class LoginCheck implements HandlerInterceptor {
	 ...
	 * logger.info("############## PRE-HANDLER ##############"); boolean pass =
	 * true;
	 * 
	 * HttpSession session = request.getSession();
	 * if(session.getAttribute("loginInfo")==null) {
	 * logger.info("[session : loginInfo ] IS NULL"); pass = false;
	 * 
	 * String encodedMessage = URLEncoder.encode("로그인이 필요한 서비스 입니다.", "UTF-8");
	 * response.sendRedirect("/?msg=" + encodedMessage); } return pass; }
	 * 
	 ...
	 * Exception { logger.info("############## POST-HANDLER ##############");
	 * 
	 * 
	 * 
	 * HttpSession session = request.getSession(); EmployeeDTO employeeDTO =
	 * (EmployeeDTO) session.getAttribute("loginInfo"); if(employeeDTO == null) {
